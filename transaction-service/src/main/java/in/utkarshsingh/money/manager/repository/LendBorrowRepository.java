...
public interface LendBorrowRepository extends JpaRepository<LendBorrowEntity, Long> {
    ...
    @Query("""
            ...
            WHERE lb.profile.id = :profileId AND lb.type = :type
            AND (:search = '' OR LOWER(lb.name) LIKE LOWER(CONCAT('%', :search, '%')) OR LOWER(lb.personName) LIKE LOWER(CONCAT('%', :search, '%')))
            AND (:status IS NULL OR lb.status = :status)
            """)
    Page<LendBorrowEntity> findAllByFilters(@Param("profileId") Long profileId, @Param("type") LendBorrowType type, @Param("search") String search, @Param("status") LendBorrowStatus status, Pageable pageable);
    @Query("""
            SELECT lb FROM LendBorrowEntity lb
            WHERE lb.profile.id = :profileId AND lb.type = :type
            ...
            AND (:search = '' OR LOWER(lb.name) LIKE LOWER(CONCAT('%', :search, '%')) OR LOWER(lb.personName) LIKE LOWER(CONCAT('%', :search, '%')))
            """)
    Page<LendBorrowEntity> findOverdueByFilters(@Param("profileId") Long profileId, @Param("type") LendBorrowType type, @Param("search") String search, @Param("today") LocalDate today, Pageable pageable);
